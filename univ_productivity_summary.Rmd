...
total_focal_uni<-authors_df %>% 
  filter(focal_uni==TRUE) %>% 
  select(authorID) %>% 
  distinct() %>% 
  tally()

total_NOTfocal<-authors_df %>% 
  filter(focal_uni==FALSE) %>% 
  select(authorID) %>% 
  distinct() %>% 
  tally() 
...
all_author_positions <- authors_df %>%
  filter(focal_uni==TRUE) %>% 
  distinct(refID,univ,.keep_all=TRUE) %>% 
  arrange(refID)
...
auth_per_pub<-authors_df %>% 
  group_by(refID) %>% 
          summarize(FocalUni=sum(focal_uni==TRUE),
          nonFocalUni=sum(focal_uni==FALSE),
...
no_FocalUni_affils_srch<-authors_df %>% select(source) %>% mutate(source=gsub("affil_","",source)) %>% mutate(source=gsub(".csv","",source)) %>% mutate(source=gsub("_2019","",source)) %>% 
...
mutate(source=gsub("_2025","",source)) %>% 
  mutate(source=gsub("_2026","",source)) %>% 
distinct() %>% 
summarize(n=n_distinct(source))



# Number of focal_uni affiliations returned

no_FocalUni_affils<-authors_df %>% filter(focal_uni==TRUE) %>% summarize(n=n_distinct(affil_id))
...
#   format = "latex",
#   align = "lcc",
#   caption = "Avg No of focal_uni and Nonfocal_uni Authors Per Publication",
#   escape = FALSE,
#   row.names = FALSE,
#   booktabs = T,
#   linesep = ""
...
- **No. of focal University SCOPUS IDs searched:** N = `r comma(no_FocalUni_affils_srch$n)`. 

- **No. of Focal University SCOPUS IDs returned:** N = `r comma(no_FocalUni_affils$n)`  
...
pubs_yr_1<-pubs_yr %>% 
  ...
  scale_y_continuous(expand = c(0, 0), breaks=seq(0,140000,by=10000))
  # scale_x_continuous(seq(2023, 2024, by=1))

ggsave("./images/univ/pubs_yr_1.png", width = 8, height = 8, units = "in")
# +
#   gghighlight(PY == 2025)
```

```{r  label = pubs_yr_1, echo = FALSE, fig.cap = "Publications per year (2019-2024)", out.width = '75%'}
knitr::include_graphics("./images/univ/pubs_yr_1.png")
```
...
pubs_mo_yr_1<-pubs_mo_yr %>% 
  ...
  labs(x = "Month", size=5)+
  labs(y = "No. of Publications", size=15)+
  geom_line(size=2) + 
  geom_point(size=2)+
  expand_limits(y = 0)+
  theme_classic()+
  scale_x_continuous( breaks=seq(1,12,by=1))+
  scale_y_continuous(expand = c(0, 0), breaks=seq(0,24000,by=2000))+
 ...
 ggsave("./images/univ/pubs_mo_yr_1.png", width = 10, height = 10, units = "in")
```
...
knitr::include_graphics("./images/univ/pubs_mo_yr_1.png")
```
...
pubs_per_quarter_2<-pubs_per_quarter %>% 
  ...
  # gghighlight(min(n) < 50)
  gghighlight(PY == 2024)
ggsave("./images/univ/pubs_per_quarter_2.png", width = 10, height = 10, units = "in")
...
knitr::include_graphics("./images/univ/pubs_per_quarter_2.png")
```
...
ggsave("./images/univ/monthly_pubs_1.png", width = 10, height = 10, units = "in")
...
knitr::include_graphics("./images/univ/monthly_pubs_1.png")
```



### Total Publications January 1-May 31 (alll institutions combined)
...
total_jan_may %>% 
  ...
  expand_limits(y = 0)+
  theme_classic()+
  annotate(geom="text", x=2023, y=(max(total_jan_may$n)-.05*max(total_jan_may$n)), label=(total_jan_may %>% filter(PY==2023) %>% select(n)),
                 color="white", size=4)+
  annotate(geom="text", x=2024, y=(max(total_jan_may$n)-.05*max(total_jan_may$n)), label=(total_jan_may %>% filter(PY==2024) %>% select(n)),
                 color="white", size=4)+
  annotate(geom="text", x=2025, y=(max(total_jan_may$n)-.25*max(total_jan_may$n)), label=(total_jan_may %>% filter(PY==2025) %>% select(n)),
           color="white", size=4)+
  annotate(geom="text", x=2025, y=(max(total_jan_may$n)-.10*max(total_jan_may$n)), label=paste("(", round((total_jan_may %>% filter(PY==2025) %>% select(perc_previous_yr)),2),"%)",sep=""),
           color="red", size=4)+
  scale_y_continuous(expand = c(0, 0), breaks=seq(0,40000,by=2000))+
  scale_y_continuous(expand = c(0, 0), breaks=seq(0,(max(total_jan_may %>% select(n))+5000),by=5000))+
  ...
  theme(axis.text.x =element_text(size = 12))
  # gghighlight(PY == 2025)
ggsave("./images/univ/fig_total_to_date_1.png", width = 6, height = 4, units = "in")
```



```{r  label = fig_total_to_date_1_fig, echo = FALSE, fig.cap = "Articles published by researchers at 10 most productive universities January 1-May 31, 2019-2025 and the percent change in 2025 productivity relative to the same time period in 2024 (red). Numbers in blue are the number of articles published in 2024 and 2025.", out.width = '75%'}
knitr::include_graphics("./images/univ/fig_total_to_date_1.png")
```
...
## 10 Non-Federal Institutions producing the most publications from 1 January 2019 to 31 May 2025.
...
```{r agency_n, fig.align='center',fig.cap="20 Univ producing the most Publications from 1 January 2019 to 31 May 2025",fig.height = 8,fig.width=6}
...
total_pubs_per_agency <- authors_df %>% 
  ...
  mutate(univ=str_to_sentence(univ)) %>% 
  mutate(univ=if_else(nchar(univ)==4,str_to_upper(univ),univ)) %>% 
  mutate(univ=if_else(univ=="Mass_general","Mass Gen Hosp",univ)) %>% 
  mutate(univ=if_else(univ=="PENN","UPenn",univ)) %>% 
  rename(Institution=univ) %>% 
  arrange(desc(Total))
# 
...
kable(
  digits = 2,
  format = "latex",
  caption = "N = 10 univeristies producing the most publications",
  align = "lcccc",
  escape = FALSE,
  row.names = FALSE,
  booktabs = F,
...
### Total Publications January - May 


```{r agency_n_decline_first1, fig.align='center',fig.cap="Percent change in publicaitons (Jan-May) relative to same period prvious year.",fig.height = 4,fig.width=6}
# 
...
agency_n_decline<-agency_n_decline_any %>% mutate(PY=as.numeric(PY))

agency_n_decline_sum<- agency_n_decline_any %>% 
...
agency_n_decline_sum<-agency_n_decline_sum %>% 
 ...
 annotate(geom="text", x=2023, y=(max(agency_n_decline_sum$n)-.05*max(agency_n_decline_sum$n)), label=(agency_n_decline_sum %>% filter(PY==2023) %>% select(n)),
                 color="white", size=4)+
  annotate(geom="text", x=2024, y=(max(agency_n_decline_sum$n)-.05*max(agency_n_decline_sum$n)), label=(agency_n_decline_sum %>% filter(PY==2024) %>% select(n)),
                 color="white", size=4)+
  annotate(geom="text", x=2025, y=(max(agency_n_decline_sum$n)-.25*max(agency_n_decline_sum$n)), label=(agency_n_decline_sum %>% filter(PY==2025) %>% select(n)),
           color="white", size=4)+
  annotate(geom="text", x=2025, y=(max(total_jan_may$n)-.10*max(total_jan_may$n)), label=paste("(", round((total_jan_may %>% filter(PY==2025) %>% select(perc_previous_yr)),2),"%)",sep=""),
           color="red", size=4)+
...
ggsave("./images/univ/agency_n_decline_sum.png", width = 6, height = 4, units = "in")
...
```{r  label = agency_n_decline_sum, echo = FALSE, fig.cap = "Number of articles Published between Jan-May by 10 universities and the percent change in 2025 productivity relative to the same time period in 2024 (red). Numbers in blue are the number of articles published in 2024 and 2025.", out.width = '75%'}
knitr::include_graphics("./images/univ/agency_n_decline_sum.png")
```
...
agency_n_decline %>%
  ...
  theme(axis.title.x =element_text(size = 14))+
  geom_hline(yintercept = 0) +
  facet_wrap(vars(univ),ncol = 3)+
  theme(strip.text = element_text(
    size = 10, color = "navy"))+
    gghighlight(PY==2025)

ggsave("./images/univ/agency_n_decline_2.png", width = 6, height = 9, units = "in")
```
...
knitr::include_graphics("./images/univ/agency_n_decline_2.png")
```
...
agency_n_decline_3<-agency_n_decline %>%
  ...
  filter(author_position=="any") %>% 
  filter(PY > 2023) %>%
  ggplot(aes(x = univ, y = perc_previous, fill = PY, group=univ)) +
  geom_bar(stat = "identity", position = "dodge") +
  ...
  theme(strip.text = element_text(
    size = 10, color = "navy"))
ggsave("./images/univ/agency_n_decline_3.png", width = 6, height = 9, units = "in")
# scale_y_continuous(expand = c(0, 0), limits = c(0, 2000))+
```
...
knitr::include_graphics("./images/univ/agency_n_decline_3.png")
```
