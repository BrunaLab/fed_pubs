pubs_per_month_cumulative <- function(pubs_mo, PY_max,PM_max) {
  ...
  pubs_mo_cum_fig<-
    ...
    scale_y_continuous(expand = c(0, 0), limits = c(0, max(pubs_mo_cumulative %>% select(cumul_pubs))+1500), 
                       breaks = seq(0, max(pubs_mo_cumulative %>% select(cumul_pubs))+1500, by = 5000))+
