...
```
<!-- `r total_pubs_per_agency_first[1,3]` -->
<!-- as.numeric(total_pubs_per_agency_first %>% filter(agency=="interior") %>% select(n)): `r as.numeric(total_pubs_per_agency_first %>% filter(agency=="interior") %>% select(n))` -->

<!-- r as.numeric(total_pubs_per_agency_first %>% filter(agency=="doe") %>% select(n)): `r as.numeric(total_pubs_per_agency_first %>% filter(agency=="doe") %>% select(n))` -->
...
```
<!-- `r total_pubs_per_agency_first` -->

<!-- `r as.numeric(total_pubs_per_agency_first %>% filter(agency=="interior") %>% select(n))` -->
...
### Total Publications January-July (only agencies with N > 3000 articles)



```{r agency_n_decline_first1, fig.align='center',fig.cap="Percent change in publicaitons (Jan-May) relative to same period prvious year. Only agencies producing over 3000 1st author publicaitons between 2019-2025 included.",out.width = '50%'}
...
```{r  label = agency_n_decline_2, echo = FALSE, fig.cap = "Percent change in articles published from previous Jan-May federal agencies or institutions producing N > 3000 articles 2019-2025.", out.width = '75%'}
