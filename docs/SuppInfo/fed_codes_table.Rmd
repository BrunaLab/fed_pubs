...
```{r setup-fedtable, include=FALSE}
library(tidyverse)
library(janitor)
# library(gghighlight)
library(kableExtra)
...
```{r fedcodes, cache=FALSE }

pull <-
  read_rds(here('./data_clean/for_pub/affils_df_analysis_fed.rds')) %>% 
  filter(federal==TRUE) %>% 
...
# pull <-
#   read_rds(here('./data_clean/affils_df_clean_20251010.rds')) %>% 
#   filter(federal==TRUE) %>% 
#   select(affil_id, affiliation,agency,agency_primary) %>% 
#   distinct() %>% 
#   mutate(affil_id = as.character(affil_id)) 
...
fed_scopus_codes<-pull %>%
  arrange(agency_primary, affiliation, affil_id) %>% 
  mutate(agency_primary = if_else(agency_primary == "Other Agencies, Commissions, or Institutions","other",agency_primary)) %>% 
  mutate(agency_primary=tolower(agency_primary)) %>% 
  mutate(agency_primary=gsub("[,]","",agency_primary)) %>% 
  mutate(affiliation=gsub("[,]","",affiliation)) %>% 
  write_csv(here("./data_archive/fed_scopus_codes.csv"))
rm(fed_scopus_codes)
...
pull %>%
  ...
  kable(
    ...
    booktabs = T,
    linesep = "",
    caption = "Scopus Affiliation ID codes used to search for publications by authors affiliated with US federal government agencies and departments. Differences in how authors or journals abbreviate or structure affiliations can result in the same affiliation having multiple ID codes. A .csv file with each individual code and its respective affiliation and agency is available for download at the projects Github repository (see Materials and Methods).",
    align = "rl",
  ) %>%
  # kable(digits = 2,
  #   format = "latex",
