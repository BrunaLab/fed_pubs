...
bs_stats_fed<-read_csv(here(paste(fed_dir,"bs_stats_all_fed.csv",sep="/")))
bs_stats_uni<-read_csv(here(paste(uni_dir,"bs_stats_all_uni.csv",sep="/")))
...
uni_summary_data<-read_csv(here(paste(uni_dir,"summary_data_uni.csv",sep="/")))
...
uni_auth_per_pub_means<-read_csv(here(paste(uni_dir,"auth_per_pub_means_uni.csv",sep="/")))

# total_papers_by_uni<-read_csv("./docs/summary_info/total_papers_by_uni.csv")
total_papers_by_uni<-read_csv(here(paste(uni_dir,"total_papers_by_uni.csv",sep="/"))) %>% 
  filter(!is.na(uni))


# total_pubs_per_uni_first<-read_csv("./docs/summary_info/uni_total_pubs_per_uni_first.csv") %>% 
  total_pubs_per_uni_first<-read_csv(here(paste(uni_dir,"uni_total_pubs_per_uni_first.csv",sep="/"))) %>% 
...
papers_by_cat_uni<-read_csv(here(paste(uni_dir,"papers_by_cat_uni.csv",sep="/")))
...
perc_change_fed<-read_csv(here(paste(fed_dir,"perc_change_agency_fed.csv",sep="/")))

final_month<-perc_change_fed %>% slice_tail(n=1) %>% select(PM)
...
We used bootstrapping [@efronIntroductionBootstrap1994] to determine if the number of articles published from January-`r final_month` 2025 with a federal first author was significantly different from the number published during this time period in previous years. Because the bootstrap sampling should reflect as closely as possible the structure of the underlying data, and the number of published articles differs by month (fig. \@ref(fig:pubs-monthly-fed)), we first used the 2019-2025 data to count the number of publications produced by each agency in each month of each year. We then simulated each agency's January-`r final_month` productivity by sampling at random with replacement from each month's publication counts and then summing the January-`r final_month` total across all agencies. This procedure was repeated N = 1000 times, after which we counted the percentage of bootstrap runs where the simulated number of total January-`r final_month` papers was lower than the actual number published in 2025. Finally, we conducted a less conservative bootstrap test in which an agency's publication counts for each month of the simulation were selected without replacement from the pooled monthly counts.

The number of January-`r final_month` publications in the 'strict' bootstrap ranged from `r comma(as.numeric(bs_stats_fed %>% filter(bs_cat=="strict") %>% select(min_bs_c_pubs)))`-`r comma(as.numeric(bs_stats_fed %>% filter(bs_cat=="strict") %>% select(max_bs_c_pubs)))` (median = `r comma(as.numeric(bs_stats_fed %>% filter(bs_cat=="strict") %>% select(median_bs_c_pubs)))`). In the less conservative simulations, the number of publications ranged from `r comma(as.numeric(bs_stats_fed %>% filter(bs_cat=="open") %>% select(min_bs_c_pubs)))`-`r comma(as.numeric(bs_stats_fed %>% filter(bs_cat=="open") %>% select(max_bs_c_pubs)))` (median = `r comma(as.numeric(bs_stats_fed %>% filter(bs_cat=="open") %>% select(median_bs_c_pubs)))`). Both the 'strict' and less-conservative tests indicate that the total number of federal first-author publications produced from January-`r final_month` 2025 is significantly lower than in previous years (`r comma(100-as.numeric(bs_stats_fed %>% filter(bs_cat=="open") %>% select(perc_below_obs)))`% of the bootstrap runs are below the observed value for 2025, fig. \@ref(fig:bs-fig)). 

We used identical bootstrapping procedures to determine if the number of January-`r final_month` 2025 publications with first authors from focal universities was significantly different from comparable productivity in previous years. In contrast, to the results for Federal first-authors, 2025 productivity by first-authors at focal universities is not significantly different from that in previous years (percentage of runs in which the number of publications was lower than that observed in 2025: `r bs_stats_uni %>% filter(bs_cat=="strict") %>% select(perc_below_obs)`% ('strict' simulations) and `r bs_stats_uni %>% filter(bs_cat=="open") %>% select(perc_below_obs)`% (less-conservative simulations), fig. \@ref(fig:bs-fig-uni)).
...
knitr::include_graphics(here(paste(uni_dir,"/bs_composite_fig_uni.png",sep="")),error=FALSE)
...
```{r  label = uni-decline-3, echo = FALSE, fig.cap = "Cumulative number of publications through July of 2019-2025 whose 1st author was affiliated with one of 12 focal universities. The solid lines are the annual values for 2019-2025; the dashed line is the 2019-2024 average. Note the different scales of the y-axes of each subplot.", out.height = '90%', out.width = '90%',fig.align='center'}
# knitr::include_graphics("images/pubs_mo_cum_uni_lines.png", error=FALSE)
knitr::include_graphics(here(paste(uni_dir,"/pubs_mo_cum_uni_lines.png",sep="")),error=FALSE)
```
